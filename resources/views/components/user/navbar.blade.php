{{-- Notifikasi refund otomatis --}}
@if(session('refund_claimed'))
        ...
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-white text-sm mt-1">
                    <strong>Rp. {{ number_format(session('refund_claimed')['total'], 0, '.', ',') }}</strong>
                    dari {{ session('refund_claimed')['count'] }} transaksi gagal telah ditambahkan ke saldo Anda.
                </p>
                <div class="mt-2 text-xs text-slate-400">
                    @foreach(session('refund_claimed')['details'] as $detail)
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center h-16 gap-6 w-full">
                ...
                <a href="{{ url('/') }}" class="flex items-center">
                    ...
                    @else
                        <div
                            ...
                            <span class="text-background-dark font-black text-xl">{{ substr(ENV('APP_NAME'), 0, 1) }}</span>
                        </div>
                    @endif
                </a>
            </div>
            ...
            <div class="hidden lg:flex items-center gap-8 flex-shrink-0 text-sm font-bold ml-4">
                <a class="{{ Request::is('/') || Request::is('id') ? 'text-white' : 'text-slate-300 hover:text-white' }} transition-colors"
                    href="{{ url('/') }}">Beranda</a>
                <a class="{{ Route::currentRouteName() == 'cari' ? 'text-white' : 'text-slate-300 hover:text-white' }} transition-colors"
                    href="{{ route('cari') }}">Cek Transaksi</a>
                <a class="{{ Route::currentRouteName() == 'leaderboardd' ? 'text-white' : 'text-slate-300 hover:text-white' }} transition-colors"
                    href="{{ route('leaderboardd') }}">Leaderboard</a>
            ...
            <div class="hidden lg:flex items-center gap-6 flex-shrink-0 ml-auto justify-end">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-white font-bold hover:text-slate-300 transition-colors whitespace-nowrap text-sm">Masuk</a>
                    <a href="{{ route('register') }}" class="px-6 py-2 rounded-full bg-white text-black font-bold hover:bg-slate-200 transition-colors whitespace-nowrap text-sm">Daftar</a>
                @endauth
            </div>
        </div>
    </div>
...
<div x-show="isMobileMenuOpen" 
    ...
    <div class="fixed inset-y-0 left-0 z-[101] w-full max-w-xs transition duration-300 transform bg-[#060B18] border-r border-white/10 shadow-2xl overflow-y-auto no-scrollbar"
        ...
        <div class="flex items-center justify-between p-6 border-b border-white/5">
            <a href="{{ url('/') }}" class="flex items-center">
                @if ($config->logo_header)
                    <img src="{{ asset($config->logo_header) }}" alt="Logo" class="h-8 w-auto">
                @else
                    <span class="text-white font-black text-xl uppercase tracking-tighter">{{ ENV('APP_NAME') }}</span>
                @endif
            </a>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
                ...
                <div class="pt-3 border-t border-white/5 flex justify-between items-center">
                    ...
                    <span class="text-sm font-black text-white">Rp {{ number_format(Auth::user()->balance, 0, ',', '.') }}</span>
                </div>
            </div>
            @endauth

            <nav class="space-y-1">
                <a href="{{ url('/') }}" class="flex items-center gap-4 p-3 rounded-xl {{ Request::is('/') || Request::is('id') ? 'bg-secondary/10 text-secondary' : 'text-slate-300 hover:bg-white/5' }} transition-all text-sm font-bold">
                    <span class="material-symbols-outlined text-xl">home</span> Beranda
                </a>
                @auth
                <a href="{{ route('dashboard') }}" class="flex items-center gap-4 p-3 rounded-xl {{ Route::currentRouteName() == 'dashboard' ? 'bg-secondary/10 text-secondary' : 'text-slate-300 hover:bg-white/5' }} transition-all text-sm font-bold">
                    <span class="material-symbols-outlined text-xl">dashboard</span> Dashboard
                </a>
                <a href="{{ route('deposit') }}" class="flex items-center gap-4 p-3 rounded-xl {{ Route::currentRouteName() == 'deposit' ? 'bg-secondary/10 text-secondary' : 'text-slate-300 hover:bg-white/5' }} transition-all text-sm font-bold">
                    <span class="material-symbols-outlined text-xl">account_balance_wallet</span> Deposit
                </a>
                @endauth
                <a href="{{ route('cari') }}" class="flex items-center gap-4 p-3 rounded-xl {{ Route::currentRouteName() == 'cari' ? 'bg-secondary/10 text-secondary' : 'text-slate-300 hover:bg-white/5' }} transition-all text-sm font-bold">
                    <span class="material-symbols-outlined text-xl">history</span> Cek Transaksi
                </a>
                <a href="{{ route('leaderboardd') }}" class="flex items-center gap-4 p-3 rounded-xl {{ Route::currentRouteName() == 'leaderboardd' ? 'bg-secondary/10 text-secondary' : 'text-slate-300 hover:bg-white/5' }} transition-all text-sm font-bold">
            ...
            <div class="space-y-3 pt-4 border-t border-white/5">
                ...
                <div class="grid grid-cols-1 gap-2">
                    <a href="{{ route('hitungwr') }}" class="flex items-center gap-4 p-3 rounded-xl text-slate-300 hover:bg-white/5 transition-all text-sm font-bold">
                        <span class="material-symbols-outlined text-xl text-pink-500">query_stats</span> Win Rate
                    </a>
                    <a href="{{ route('hitungpointmw') }}" class="flex items-center gap-4 p-3 rounded-xl text-slate-300 hover:bg-white/5 transition-all text-sm font-bold">
                        <span class="material-symbols-outlined text-xl text-purple-500">Auto_Fix_High</span> Magic Wheel
                    </a>
                    <a href="{{ route('sus.index') }}" class="flex items-center gap-4 p-3 rounded-xl bg-secondary/10 text-secondary border border-secondary/20 transition-all text-sm font-bold">
            ...
            <div class="pt-6">
                ...
                @else
                    <div class="grid grid-cols-2 gap-3">
                        <a href="{{ route('login') }}" class="flex items-center justify-center p-3 rounded-xl bg-white/5 border border-white/10 text-white font-bold text-sm">Masuk</a>
                        <a href="{{ route('register') }}" class="flex items-center justify-center p-3 rounded-xl bg-secondary text-white font-bold text-sm shadow-lg shadow-secondary/20">Daftar</a>
                    </div>
                @endauth
            </div>
        </div>
...
<div id="searchModal" 
    ...
    <main class="relative z-10 w-full max-w-2xl">
        <div class="search-container bg-[rgba(10,10,15,0.9)] rounded-[2rem] border border-[#00E5FF]/20 shadow-2xl overflow-hidden relative" style="backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);">
            ...
            <div class="p-6">
                <form action="{{ route('cari') }}" method="GET" class="relative group">
            ...
            <div id="defaultSearchState" class="px-6 pb-6 animate-fade-in-up">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ url('/') }}" class="text-[10px] text-gray-400 hover:text-[#00E5FF] transition-colors tracking-wider uppercase" style="font-family: 'Orbitron', sans-serif;">View All</a>
                </div>
                
                <div class="flex overflow-x-auto sm:grid sm:grid-cols-4 gap-4 sm:gap-6 pt-8 pb-4 -mx-2 px-2 no-scrollbar snap-x">
                    @isset($kategoris)
                        @php $populerCount = 0; @endphp
                        @foreach ($kategoris as $category)
                            @if ($category->tipe == 'populer' && $populerCount < 8)
                                @php
                                    ...
                                    $populerCount++;
                                @endphp
                                <a href="{{ url('/id/'.$category->kode) }}" class="group relative flex flex-col items-center cursor-pointer flex-shrink-0 w-[90px] sm:w-auto snap-center" style="--theme-color: {{ $themeColor }};">
                                    <div class="relative w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                                        ...
                                        <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-full overflow-hidden relative z-10 border border-white/10 group-hover:border-transparent">
                                            <img alt="{{ $category->nama }}" class="w-full h-full object-cover transform transition-transform duration-500 group-hover:scale-110" src="{{ $category->thumbnail }}"/>
                                        </div>
                                    </div>
                                    <div class="mt-3 text-center relative z-20">
                                        <h3 class="text-white font-bold text-xs sm:text-sm tracking-wide transition-colors group-hover:text-[var(--theme-color)]" style="font-family: 'Orbitron', sans-serif; color: white;" onmouseover="this.style.color='var(--theme-color)'" onmouseout="this.style.color='white'">{{ $category->nama }}</h3>
                                        <p class="text-[10px] text-gray-500 uppercase tracking-wider group-hover:text-gray-300">{{ $category->sub_nama ?? 'Developer' }}</p>
                                    </div>
            ...
            <div id="activeSearchState" class="px-2 pb-6 hidden animate-fade-in-up">
                <div class="custom-scrollbar max-h-[450px] overflow-y-auto px-4">
                    ...
                    <!-- Mobile Legends -->
                    <a href="{{ url('/id/mobile-legends') }}" class="group flex items-center p-4 rounded-2xl cursor-pointer hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/10">
                    ...
                    <!-- Free Fire -->
                    <a href="{{ url('/id/free-fire') }}" class="group flex items-center p-4 rounded-2xl cursor-pointer hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/10">
                        <div class="relative flex-shrink-0">
                    ...
                    <!-- Genshin Impact -->
                    <a href="{{ url('/id/genshin-impact') }}" class="group flex items-center p-4 rounded-2xl cursor-pointer hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/10">
                        <div class="relative flex-shrink-0">
                    ...
                    <a href="{{ url('/id/sausage-man') }}" class="group flex items-center p-4 rounded-2xl cursor-pointer hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/10">
                        <div class="relative flex-shrink-0">
                            <div class="absolute inset-0 bg-[#00E5FF]/20 rounded-full blur-md opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <div class="w-14 h-14 rounded-full overflow-hidden border border-white/10 relative z-10">
                    ...
                    <!-- Valorant -->
                    <a href="{{ url('/id/valorant') }}" class="group flex items-center p-4 rounded-2xl cursor-pointer hover:bg-white/5 transition-all duration-200 border border-transparent hover:border-white/10">
                        <div class="relative flex-shrink-0">
            ...
            <div class="border-t border-white/5 bg-black/40 px-8 py-4 flex justify-between items-center text-[10px] text-gray-500 tracking-widest uppercase" style="font-family: 'Orbitron', sans-serif;">
                <div class="flex gap-6">
                    {{-- <span class="hover:text-[#00E5FF] cursor-pointer transition-colors">History</span>
                    <span class="hover:text-[#00E5FF] cursor-pointer transition-colors">Discover</span> --}}
                </div>
                <div class="flex items-center gap-3">
