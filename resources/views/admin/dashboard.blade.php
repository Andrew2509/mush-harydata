...
{{-- ============================================================
     PREMIUM ADMIN DASHBOARD — GAME TOPUP REDESIGN
     ============================================================ --}}
...
<div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="stat-card cyan">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value">Rp {{ number_format($total_pembelian, 0, '.', '.') }}</div>
                    <div class="stat-sub"><i class="fas fa-shopping-cart"></i> {{ $banyak_pembelian }} Pesanan Hari Ini</div>
                </div>
                <div class="stat-icon">
        ...
        <div class="stat-card green">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value">Rp {{ number_format($total_pembelian_success, 0, '.', '.') }}</div>
                    <div class="stat-sub" style="color:#39ff14;"><i class="fas fa-check-circle"></i> {{ $banyak_pembelian_success }} Sukses</div>
                </div>
        ...
        <div class="stat-card">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value">Rp {{ number_format($total_pembelian_pending, 0, '.', '.') }}</div>
                    <div class="stat-sub"><i class="fas fa-clock"></i> {{ $banyak_pembelian_pending }} Menunggu</div>
                </div>
                <div class="stat-icon">
        ...
        <div class="stat-card red">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value">Rp {{ number_format($total_pembelian_batal, 0, '.', '.') }}</div>
                    <div class="stat-sub" style="color:#ff3366;"><i class="fas fa-times-circle"></i> {{ $banyak_pembelian_batal }} Gagal</div>
                </div>
        ...
        <div class="stat-card cyan">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value" style="font-size:1.2rem;">Rp {{ number_format($total_keseluruhan_pembelian, 0, '.', '.') }}</div>
                    <div class="stat-sub">{{ $banyak_keseluruhan_pembelian }}x pemesanan</div>
                </div>
        ...
        <div class="stat-card">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value" style="font-size:1.2rem;">Rp {{ number_format($total_keseluruhan_pembelian_berhasil, 0, '.', '.') }}</div>
                    <div class="stat-sub">{{ $banyak_keseluruhan_pembelian_berhasil }}x pemesanan</div>
                </div>
        ...
        <div class="stat-card">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value" style="font-size:1.2rem;">Rp {{ number_format($total_keseluruhan_pembelian_pending, 0, '.', '.') }}</div>
                    <div class="stat-sub">{{ $banyak_keseluruhan_pembelian_pending }}x pemesanan</div>
                </div>
        ...
        <div class="stat-card">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value" style="font-size:1.2rem;">Rp {{ number_format($total_keseluruhan_pembelian_batal, 0, '.', '.') }}</div>
                    <div class="stat-sub">{{ $banyak_keseluruhan_pembelian_batal }}x pemesanan</div>
                </div>
    ...
    {{-- Keuntungan Bersih (highlight card) --}}
        ...
        <div class="stat-card cyan" style="max-width:400px; border-color: rgba(0,255,255,0.2) !important; background: rgba(0,255,255,0.03) !important;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div>
                    ...
                    <div class="stat-value" style="font-size:1.5rem;">Rp {{ number_format($keuntungan_bersih, 0, '.', '.') }}</div>
                    <div class="stat-sub">
    ...
    <div class="dash-grid-3 mb-4">
        <div class="quick-stat-card">
            ...
            <div class="qs-value">{{ $total_users }}</div>
        ...
        <div class="quick-stat-card">
            ...
            <div class="qs-value">{{ $total_services }}</div>
    ...
    <div class="tx-table-wrap mb-4">
        <div class="tx-table-header">
            ...
            <a href="{{ route('pesanan') }}" class="btn-print">
        ...
        <div style="overflow-x:auto;">
            <table class="tx-table" id="recentTxTable">
                ...
                <tbody id="recentTxBody">
                    {{-- Rows populated via Blade if data available, otherwise JS sample --}}
                    @if(isset($recent_transactions) && count($recent_transactions) > 0)
                        @foreach($recent_transactions as $tx)
                        <tr>
                            <td style="font-family:'Orbitron',sans-serif; font-size:0.72rem; color:var(--accent-cyan);">{{ $tx->order_id ?? $tx->id }}</td>
                            <td>
                                <div style="font-weight:600;">{{ $tx->nickname ?? 'Player' }}</div>
                                <div style="font-size:0.65rem; color:var(--text-muted);">{{ $tx->user_id }}{{ $tx->zone ? ' ('.$tx->zone.')' : '' }}</div>
                            </td>
                            <td>{{ $tx->layanan ?? '-' }}</td>
                            <td style="font-size:0.78rem; color:var(--text-secondary);">{{ $tx->nickname ?? '-' }}</td>
                            <td style="font-weight:600;">Rp {{ number_format($tx->harga ?? 0, 0, '.', '.') }}</td>
                            <td>
                                @php
                                    $statusClass = 'pending';
                            ...
                            <td style="font-size:0.78rem; color:var(--text-muted);">{{ $tx->created_at ? \Carbon\Carbon::parse($tx->created_at)->format('d M Y H:i') : '-' }}</td>
                        </tr>
                        @endforeach
                    @else
                        {{-- Demo/placeholder rows if no data --}}
                        <tr>
    ...
    <div class="dash-grid-5 mb-4">
        @if(isset($popular_games) && count($popular_games) > 0)
            ...
            <div class="game-card">
                <div class="game-icon">
                    @if(isset($game->thumbnail) && $game->thumbnail)
                        <img src="{{ $game->thumbnail }}" alt="{{ $game->nama }}">
                    @else
                        <i class="fas fa-gamepad"></i>
                    @endif
                </div>
                <div class="game-name">{{ $game->nama ?? 'Game' }}</div>
                <div class="game-topups">{{ number_format($game->total_topups ?? 0, 0, '.', '.') }} topups</div>
            </div>
            @endforeach
        @endif
    </div>
...
<script>
$(function () {
    var grafikData = {!! $grafik_chart !!};
    var visitorData = {!! $visitor_chart !!};
