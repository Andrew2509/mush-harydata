...
<main class="relative z-10 flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 md:py-8 pb-24 md:pb-8 flex flex-col md:flex-row gap-6 md:gap-8">
    ...
    <section class="flex-grow space-y-4 md:space-y-6 min-w-0">
        ...
        <div class="mb-6 md:mb-8">
            <h1 class="text-2xl md:text-3xl font-display font-bold text-white mb-1 md:mb-2 neon-text">Riwayat Transaksi</h1>
                            ...
                            @foreach($data as $pesanan)
                                @if($pesanan->tipe_transaksi !== 'joki')
                                    ...
                                    <tr class="hover:bg-white/5 transition-colors">
                                        ...
                                        <td class="px-6 py-4 font-mono text-xs text-slate-400">n/a</td>
                                        <td class="px-6 py-4 text-xs">{{ $pesanan->layanan }}</td>
                                        <td class="px-6 py-4 font-mono text-xs">{{ $pesanan->user_id }}{{ $zone }}</td>
                                        <td class="px-6 py-4 font-medium text-white">Rp&nbsp;{{ number_format($pesanan->harga, 0, ',', '.') }}</td>
                                ...
                                @else
                                    @foreach($joki as $jokis)
                                        @if($jokis->order_id == $pesanan->order_id)
                                            @php
                                                $zone = $pesanan->zone != null ? "-".$pesanan->zone : "";
                                                $statusJoki = $jokis->status_joki;
                                                $statusClassJoki = ($statusJoki == 'Sukses') ? 'status-success' : 'status-cancelled';
                                            @endphp
                                            <tr class="hover:bg-white/5 transition-colors">
                                                <td class="px-6 py-4 font-medium text-white whitespace-nowrap">
                                                    <a href="{{ ENV('APP_URL') }}/id/invoices/{{ $pesanan->order_id }}" class="text-cyan-400 hover:text-cyan-300 transition-colors">{{ $pesanan->order_id }}</a>
                                                ...
                                                <td class="px-6 py-4 text-xs text-slate-400">{{ $pesanan->created_at }}</td>
                                                <td class="px-6 py-4">
                                                    <span class="status-pill {{ $statusClassJoki }}">{{ $statusJoki }}</span>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-slate-600 text-xs">Tidak ada aktivitas transaksi.</p>
                                    </div>
                                </td>
                            </tr>
                        @endif
        ...
        <div class="md:hidden space-y-3">
            @if(count($data) > 0)
                @foreach($data as $pesanan)
                    @if($pesanan->tipe_transaksi !== 'joki')
                        ...
                        @endphp
                        <a href="{{ ENV('APP_URL') }}/id/invoices/{{ $pesanan->order_id }}" class="block glass-panel rounded-xl p-4 border border-white/5 hover:border-cyan-500/30 transition-all active:scale-[0.98]">
                            ...
                            <div class="flex items-center justify-between text-xs">
                                <span class="text-slate-500 font-mono">{{ $pesanan->user_id }}{{ $zone }}</span>
                        ...
                        @foreach($joki as $jokis)
                            @if($jokis->order_id == $pesanan->order_id)
                                ...
                                <a href="{{ ENV('APP_URL') }}/id/invoices/{{ $pesanan->order_id }}" class="block glass-panel rounded-xl p-4 border border-white/5 hover:border-cyan-500/30 transition-all active:scale-[0.98]">
                                    <div class="flex items-center justify-between mb-3">
                                        <span class="text-cyan-400 font-semibold text-xs truncate mr-2">{{ $pesanan->order_id }}</span>
                                        <span class="status-pill {{ $statusClassJoki }} flex-shrink-0">{{ $statusJoki }}</span>
                                    </div>
                                    <p class="text-white font-medium text-sm mb-2 truncate">{{ $pesanan->layanan }}</p>
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-slate-500 font-mono">{{ $pesanan->user_id }}{{ $zone }}</span>
            ...
            @else
                <div class="glass-panel rounded-xl p-8 text-center">
                    ...
                    <p class="text-slate-600 text-xs mt-1">Tidak ada aktivitas transaksi.</p>
                </div>
            @endif
        </div>
    </section>
